...
Yulu 不需要账号。录音文件和任务状态保存在你的 Mac 上。启用 Agent 处理后，Yulu
会把音频交给 Hermes；处理完全在本地还是使用远程服务，由你的 Hermes 配置决定。
...
- 默认只显示原文。可在「设置 → 转写」安装本地模型获得低延时私有字幕；最终转录仍由 Hermes 生成。
...
- Python 3.10 或更高版本。
- Yulu 的 LaunchAgent 能找到可用的 Hermes CLI，用于录音转写、纪要与语音输入。
...
- **Hermes** 负责录音转写、生成纪要，以及经过明确授权的 Notion 投递。
...
- 启用自动处理代表允许 Yulu 把音频交给 Hermes；处理在本地还是远程由 Hermes provider
  配置决定。
