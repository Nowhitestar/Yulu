...
completed recording as durable local work, and delegates recording intelligence
to Hermes.
...
- Hermes owns recording speech recognition, summaries, and explicitly
...
Do not add a second speech engine, summary worker, chat engine, connector
executor, or file-only work queue to Yulu. The accepted decision is
[`ADR-005`](yulu/spec/adr/005-agent-native-durable-recording-pipeline.md).

### Constraints

- **Platform:** macOS 13+ is the shipped capture platform.
- **Privacy:** raw capture is local; Agent processing follows Hermes' provider
  configuration and must not be described as unconditionally on-device.
...
- Hermes CLI for automatic recording processing and voice transcription.
- `ffmpeg`/`sox` for audio inspection and transport preparation.
- Optional `gog`/`cloudflared` only for Yulu-owned calendar scheduling.

Yulu does not install or import a speech model runtime. Connector credentials
...
Relevant config sections are `audio`, `transcription` (language and dictation
context only), `agent_pipeline`, `llm`, `agent_console`, `status_agent`,
...
- `recordingPipeline.ts` owns admission, claims, state transitions, recovery,
  and Hermes dispatch.
- `artifactStore.ts` validates the fixed staging pair and atomically replaces
  final sidecars.
- `agentGateway.ts` is the Hermes audio/workflow boundary and audits required
  Host tool calls.
...
- `llm.agent.provider` selects the general Agent only; recording and dictation
  remain Hermes-backed.
...
Python completion adapter -> authenticated loopback Host
                 ...
                 +-> atomic event spool ---+
                                           |
                                           v
                              Hermes transcription + summary
                                           |
                                           v
                         Host commits transcript + summary together
                                           |
...
| `recordingPipeline.ts` | Validation, idempotency, claims, Hermes dispatch, recovery |
| `agentGateway.ts` | Hermes audio service, workflow, and required-tool audit |
| `artifactStore.ts` | Task staging and atomic transcript/summary commit |
...
3. Hermes writes fixed staging files `transcript.txt` and `summary.md`; the Host
   validates and commits them as one logical operation with hashes/provenance.
4. Notion can begin only after artifacts are committed and the task opted in.
...
```text
queued -> running -> artifacts_committed -> completed
   |          |              |
...
- ADR-005 is the current runtime decision.
- It supersedes ADR-001, ADR-003, and the execution decision in ADR-004.
- ADR-002 remains relevant for glossary data; Hermes consumes that context.
- Historical ADR bodies remain history and must not be treated as active
  implementation instructions.

### Anti-patterns

- Reintroducing a Yulu-owned speech/model provider.
