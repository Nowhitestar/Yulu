...
minimum macOS version metadata repair includes `Release-As: 0.23.0-rc.13`, so the rolling
Release PR advances the acceptance line to RC13 instead of opening a new minor
...
`v0.23.0` may be pushed only while `VERSION` is `0.23.0-rc.13` and the local
`v0.23.0-rc.13` tag resolves to the same source commit. That stable promotion uses
...
## RC13 public-DMG acceptance controller
...
The `v0.23.0-rc.13` public-DMG harness is a clean-target collector, not an
...
python3 packaging/acceptance/validate_returned_public_dmg.py \
  ...
  --tag v0.23.0-rc.13 \
  --dmg /private/path/to/yulu-macos-arm64-v0.23.0-rc.13.dmg \
...
`refs/tags/v0.23.0-rc.13` commit, the public DMG/checksum bytes, bundle/signature
...
v0.22.2 migration or the RC13-to-stable journey. Keep #169's broader release
guidance separate from this RC13 evidence procedure.
