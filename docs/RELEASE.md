...
macOS 26 clean-host shim repair includes `Release-As: 0.23.0-rc.7`, so the rolling
Release PR advances the accepted line to RC7 instead of opening a new minor
...
`v0.23.0` may be pushed only while `VERSION` is `0.23.0-rc.7` and the local
`v0.23.0-rc.7` tag resolves to the same source commit. That stable promotion uses
...
## RC7 public-DMG acceptance controller

The `v0.23.0-rc.7` public-DMG harness is a clean-target collector, not an
...
python3 packaging/acceptance/validate_returned_public_dmg.py \
  ...
  --tag v0.23.0-rc.7 \
  --dmg /private/path/to/yulu-macos-arm64-v0.23.0-rc.7.dmg \
...
`refs/tags/v0.23.0-rc.7` commit, the public DMG/checksum bytes, bundle/signature
...
#171; do not substitute an RC6-to-RC7 update for either the required v0.22.2
migration or the RC7-to-stable journey. Keep #169's broader release guidance
separate from this RC7 evidence procedure.
