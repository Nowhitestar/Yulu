...
This release candidate supersedes RC6 for Yulu's public drag-to-Applications
acceptance on Apple Silicon Macs.

- **clean macOS acceptance** — distinguish Apple's inert `/usr/bin/python3`
  and `/usr/bin/pip3` Xcode-select shims from usable host Python tooling while
  still rejecting actual host Python, Node, package managers, Xcode, or Command
  Line Tools.
...
`yulu-macos-arm64-v0.23.0-rc.7.dmg`, drag `Yulu.app` to `/Applications`, and
