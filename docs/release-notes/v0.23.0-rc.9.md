...
This release candidate supersedes RC8 after clean-host validation exposed a
fresh-install Host bootstrap failure on Apple Silicon Macs.

- **fresh Host bootstrap** — create the standard per-user configuration with
  private product defaults when neither current nor legacy configuration exists.
- **safe recovery** — publish the fresh configuration only after durable-data
  preparation succeeds, preserve an existing or migrated configuration, and
  remain idempotent across Host restarts.
...
`yulu-macos-arm64-v0.23.0-rc.9.dmg`, drag `Yulu.app` to `/Applications`, and
