...
- **D-02b [spike exit criteria, PROV-02]:** the spike's pass bar is the FAILURE paths, not the happy path: (1) **kill-at-step-N resume** (PROV-04) and (2) **tampered-asset rejection** (PROV-03). Both are testable WITHOUT a real agent (simulate a kill mid-run; corrupt an asset). `uv`/`uvx`: evaluate, recommend DEFER (D-07).
