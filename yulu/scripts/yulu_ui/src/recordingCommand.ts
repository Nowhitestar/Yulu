...
export async function runRecordAudio(
  ...
  const { stdout, stderr } = await exec("python3", [join(ctx.paths.scriptDir, "record_audio.py"), ...args], {
    env: {
      ...
      YULU_APPLICATION_SUPPORT_DIR: ctx.paths.durableDataDir,
      YULU_CACHE_DIR: ctx.paths.cacheDir,
      YULU_IPC_DIR: ctx.paths.ipcDir,
      YULU_LOG_DIR: ctx.paths.logsDir,
      YULU_MEDIA_LIBRARY_DIR: ctx.paths.mediaLibraryDir,
      YULU_LEGACY_READ_ONLY_DATA_DIR: ctx.paths.legacyReadOnlyDataDir,
      YULU_CONFIG_DIR: undefined,
    },
