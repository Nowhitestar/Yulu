...
export function buildHermesRecordingPrompt(input: AgentArtifactWorkflowInput): string {
  ...
  return [
    ...
    `Selected Summary Provider: ${task.summaryProvider}`,
    `Selected summary model: ${task.summaryModel}`,
    ...
    "4. Call `recording_artifact_commit` with taskId, leaseToken, summaryProvider, summaryModel, and provenance. The identity must exactly match the selected values above; Yulu validates it before replacing any committed summary.",
    "",
