...
export function AudioSection({ tracker }: AudioSectionProps) {
  ...
  return (
    <section id="audio" className="settings-section">
      ...
      <InlineEditRow label={t("settings.audio.micDevice.label")} type="select" value={micValue} options={micOptions} help={t("settings.audio.micDevice.help")} onCommit={commit("audio.mic_device") as (v: string) => void} disabled={isBlocked("audio.mic_device")} status={tracker.statusFor("audio.mic_device")} />
