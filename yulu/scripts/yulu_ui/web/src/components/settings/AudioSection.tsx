...
export function AudioSection({ tracker }: AudioSectionProps) {
  ...
  return (
    <section id="audio" className="settings-section">
      ...
      <InlineEditRow label="Microphone device" type="select" value={cfg.audio.mic_device ?? ""} options={micOpts.length ? micOpts : [{value: cfg.audio.mic_device ?? "", label: "(no devices found)"}]} help="system default input" onCommit={commit("audio.mic_device") as (v: string) => void} status={tracker.statusFor("audio.mic_device")} />
      <InlineEditRow label="System audio device" type="select" value={cfg.audio.system_audio_device ?? ""} options={sysOpts} help="ScreenCaptureKit channel" onCommit={(v) => updateMut.mutateAsync({ key: "audio.system_audio_device", value: v || null })} status={tracker.statusFor("audio.system_audio_device")} />
      <InlineEditRow label="Output directory" type="path" mode="folder" value={cfg.audio.output_dir} onCommit={commit("audio.output_dir") as (v: string) => void} status={tracker.statusFor("audio.output_dir")} />
      ...
      <InlineEditRow label="Silence duration" type="number" min={1} step={1} value={cfg.audio.silence_duration_sec} help="seconds" onCommit={commit("audio.silence_duration_sec") as (v: number) => void} status={tracker.statusFor("audio.silence_duration_sec")} />
