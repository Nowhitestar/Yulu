...
def make_release_repository(tmp_path: Path) -> Path:
    ...
    repository.mkdir()
    (repository / "VERSION").write_text("0.23.0-rc.12\n", encoding="utf-8")
    ...
    assert git(repository, "commit", "-qm", "chore: release 0.23.0-rc.12").returncode == 0
    assert git(repository, "tag", "v0.23.0-rc.12").returncode == 0
...
def test_release_identity_accepts_rc12_and_reserves_the_next_build_for_stable(tmp_path: Path):
    repository = make_release_repository(tmp_path)

    rc = resolve(repository, "v0.23.0-rc.12")
...
def test_release_identity_rejects_missing_and_mismatched_requested_tags(tmp_path: Path):
    repository = make_release_repository(tmp_path)
    assert git(repository, "tag", "-d", "v0.23.0-rc.12").returncode == 0

    missing = resolve(repository, "v0.23.0-rc.12")
    ...
    mismatched = resolve(repository, "v0.23.0-rc.12")
...
def test_public_guidance_matches_current_install_provider_and_share_boundaries():
    ...
    release_notes = (
        (ROOT / "docs" / "release-notes" / "v0.23.0-rc.12.md").read_text(encoding="utf-8")
    ...
    assert 'placeholder: "Yulu 0.23.0-rc.12"' in issue_template
    operations = (ROOT / "docs" / "operations.md").read_text(encoding="utf-8")
    assert "For the `v0.23.0-rc.12` public-DMG acceptance harness" in operations
