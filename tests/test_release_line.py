...
def make_release_repository(tmp_path: Path) -> Path:
    ...
    assert git(repository, "commit", "-qm", "chore: release 0.23.0-rc.15").returncode == 0
...
def test_release_identity_accepts_rc15_and_reserves_the_next_build_for_stable(tmp_path: Path):
    repository = make_release_repository(tmp_path)

    rc = resolve(repository, "v0.23.0-rc.15")
...
def test_release_identity_rejects_missing_and_mismatched_requested_tags(tmp_path: Path):
    repository = make_release_repository(tmp_path)
    assert git(repository, "tag", "-d", "v0.23.0-rc.15").returncode == 0

    missing = resolve(repository, "v0.23.0-rc.15")
    ...
    mismatched = resolve(repository, "v0.23.0-rc.15")
...
def test_previous_candidate_cannot_be_promoted_after_acceptance_moves_to_rc15(tmp_path: Path):
    repository = make_release_repository(tmp_path)
...
def test_public_guidance_matches_current_install_provider_and_share_boundaries():
    ...
    release_notes = (
        (ROOT / "docs" / "release-notes" / "v0.23.0-rc.15.md").read_text(encoding="utf-8")
    ...
    assert 'placeholder: "Yulu 0.23.0-rc.15"' in issue_template
    operations = (ROOT / "docs" / "operations.md").read_text(encoding="utf-8")
    assert "For the `v0.23.0-rc.15` public-DMG acceptance harness" in operations
