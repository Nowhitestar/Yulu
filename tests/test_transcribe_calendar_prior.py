"""Phase 13 Task 2 — calendar-attendee prior resolution in transcribe.py.

The Phase-12 free prior (COUNT-01). Proves transcribe._resolve_attendee_count maps a recording to
its attendee count via meeting_id (recording state) or title (schedule.json), and degrades to None
on any miss/error — the graceful default the count strategy expects. No network, no gog.
"""
...
def test_resolve_by_meeting_id(tmp_path):
    ...
    n = transcribe._resolve_attendee_count(
